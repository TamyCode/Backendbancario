...
import com.upc.demopapaytf.dtos.QuantityTypeMovementDTO;
...
public class MovementController {
    ...
    public List<MovementbyDateDTO> quantityMovementbyDateService(){
        ...
        for(String[] columna:filaLista){
            ...
            dto.setDateMovement(LocalDate.parse(columna[0]));
            dto.setQuantityMovement(Integer.parseInt(columna[1]));
            dtoLista.add(dto);
        }
        return dtoLista;
    }

    @GetMapping("/cantidadeMovimientoportipo")
    public List<QuantityTypeMovementDTO> QuantityTypeMovement(){
        List<String[]> filaLista= mS.QuantityTypeMovement();
        List<QuantityTypeMovementDTO> dtoLista=new ArrayList<>();
        for(String[] columna:filaLista){
            QuantityTypeMovementDTO dto=new QuantityTypeMovementDTO();
            dto.setTypemovement(columna[0]);
            dto.setCountMovement(Integer.parseInt(columna[1]));
