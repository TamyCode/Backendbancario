...
public class ServiceController {
    ...
    public List<ServicebyDateServiceDTO> cantidadDeServiciosporFecha(){
        ...
        for(String[] columna:filaLista){
            ...
            dto.setDateService(LocalDate.parse(columna[0]));
            dto.setQuantityDateService(Integer.parseInt(columna[1]));
            dtoLista.add(dto);
        }
        return dtoLista;
    }

    @GetMapping("/cantidadDeServiciosporusuario")
    @PreAuthorize("hasAuthority('ADMIN')")
    public List<ServicebyNameDTO> cantidadDeServiciosporNombre(){
        List<String[]> filaLista=sS.quantityServicebyName();
